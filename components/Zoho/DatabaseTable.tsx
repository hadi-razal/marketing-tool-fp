...
export const DatabaseTable = () => {
    ...
    const fetchData = async (reset = false) => {
        ...
        try {
            ...
            const res = await zohoApi.getRecords('Database_Report', undefined, from, LIMIT);
            if (res.code === 3000) {
                if (reset) {
                    setData(res.data);
                    setPage(1);
    ...
    const filteredData = data.filter(item =>
        item.Show?.toLowerCase().includes(search.toLowerCase()) ||
        item.Booth_No?.toLowerCase().includes(search.toLowerCase())
    );
