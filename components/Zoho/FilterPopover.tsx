            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="absolute top-full right-0 mt-2 w-80 bg-white border border-zinc-200 rounded-2xl shadow-2xl shadow-zinc-950/15 z-50 overflow-hidden flex flex-col max-h-[600px]"
                    >
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-1">
                                        {availableContinents.map(continent => (
                                            <label key={continent} className="flex items-center gap-3 p-2 rounded-lg hover:bg-orange-50 cursor-pointer group transition-colors">
                                                <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${tempContinents.includes(continent) ? 'bg-orange-500 border-orange-500' : 'border-zinc-300 group-hover:border-orange-400'}`}>
                                                    {tempContinents.includes(continent) && <Check className="w-3 h-3 text-white" />}
                                                </div>
                                                <span className={`text-sm ${tempContinents.includes(continent) ? 'text-zinc-950 font-medium' : 'text-zinc-600 group-hover:text-zinc-950'}`}>
                                                    {continent || 'Unknown'}
                                                </span>
                                                <input
                                                    type="checkbox"
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white border border-zinc-200 rounded-xl pl-9 pr-3 py-2 text-xs text-zinc-950 focus:outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100 transition-all placeholder:text-zinc-400"
                                    />
                                </div>
                                <div className="space-y-1 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                                    {filteredCountries.map(country => (
                                        <label key={country} className="flex items-center gap-3 p-2 rounded-lg hover:bg-orange-50 cursor-pointer group transition-colors">
                                            <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${tempCountries.includes(country) ? 'bg-orange-500 border-orange-500' : 'border-zinc-300 group-hover:border-orange-400'}`}>
                                                {tempCountries.includes(country) && <Check className="w-3 h-3 text-white" />}
                                            </div>
                                            <span className={`text-sm ${tempCountries.includes(country) ? 'text-zinc-950 font-medium' : 'text-zinc-600 group-hover:text-zinc-950'}`}>
                                                {country || 'Unknown'}
                                            </span>
                                            <input
                                                type="checkbox"
                                    ...
                                    {filteredCountries.length === 0 && (
                                        <p className="text-xs text-zinc-500 text-center py-2">No countries found</p>
                                    )}
                                </div>
                            </div>
                        </div>
